...
class Model(Container):
    ...
    def _fit_loop_rbf(self, f, ins, out_labels=[], batch_size=32,
                  nb_epoch=100, verbose=1, callbacks=[],
                  val_f=None, val_ins=None, shuffle=True,
                  callback_metrics=[], kern='gau', nu = 0.5):
        '''Abstract fit function for f(ins).
        Assume that f returns a list, labeled by out_labels.
        ...
        # Arguments
            ...
            callback_metrics: list of strings, the display names of the metrics
                ...
                concatenation of list the display names of the outputs of
                 `f` and the list of display names of the outputs of `f_val`.
            kern: type of kernel function, it could be any kernel in 
                ['gau', 'lap', 'lin', 'cos', 'qua', 'sec'] which represent
                gaussian, laplace, linear, cosine, quadratic, and secant 
                functions respectively: 
            nu = recall confidence, RbF scheduler estimates the maximum delay 
                such that instances can be recalled with this confidence in 
                the future iterations, nu takes a value in (0,1)               

        # Returns
            `History` object, fraction of instances used for training per epoch
        '''
        do_validation = False
        if val_f and val_ins:
            do_validation = True
        ...
        for x in index_array:
            daleys[x] = 1. 
        for epoch in range(nb_epoch):                
            cur_q = np.array([x for x in daleys if daleys[x] <= 1.])            
            dey_q = np.array([x for x in daleys if daleys[x] > 1.])           
            if epoch == nb_epoch - 1:
                cur_q = np.array([x for x in daleys])            
            
            q_size = len(cur_q)
            n_items_reviewed += q_size                                 
            if q_size == 0:
                continue        
            ...
            if shuffle == 'batch':
                cur_q = batch_shuffle(cur_q, batch_size)
            elif shuffle:
                np.random.shuffle(cur_q)

            cur_loss=[]
            cur_accs=[]
            epoch_logs = {}
            batches = make_batches(q_size, batch_size)
            for batch_index, (batch_start, batch_end) in enumerate(batches):
                batch_ids = cur_q[batch_start:batch_end]
                try:
                    if type(ins[-1]) is float:
                        # do not slice the training phase flag
                        ins_batch = slice_X(ins[:-1], batch_ids) + [ins[-1]]
                ...
                cur_loss += [outs[0]] * len(batch_ids)
                cur_accs += [outs[1]] * len(batch_ids)
                ...
                if batch_index == len(batches) - 1:  # last batch
                    ...
                    if do_validation:
                        # replace with self._evaluate
                        val_outs, val_loss, val_accs = self._test_loop_rbf(val_f, val_ins,
                                                   batch_size=batch_size,
                                                   verbose=0)
                        val_accs = np.array([a if a < 1. else .9 for a in val_accs])
                        if type(val_outs) != list:
                            val_outs = [val_outs]
                        # same labels assumed
                        for l, o in zip(out_labels, val_outs):
                        ...
                        ### compute optimal tau using validation data ###
                        d = 1.0
                        val_strength = np.mean(val_accs)
                        x = d / val_strength                        
                        tau = self.get_optimal_tau_rbf(kern, x, val_accs, val_loss, nu)
                            
                        ### compute delay for each instance in current batch using the above delta values ###
                        cur_accs = np.array([a if a < 1. else .9 for a in cur_accs]) 
                        if kern == 'gau':
                            nu_gau = np.sqrt(-np.log(nu) / tau)
                            for i in range(len(cur_q)):
                                if cur_accs[i] >= nu:
                                    daleys[cur_q[i]] = max(1., val_strength * nu_gau / cur_loss[i])
                        
                        if kern == 'lap':
                            nu_lap = np.log(nu)    
                            for i in range(len(cur_q)):
                                if cur_accs[i] >= nu:
                                    daleys[cur_q[i]] = max(1., -1. * val_strength * nu_lap / (cur_loss[i] * tau))
                        
                        if kern == 'lin':
                            nu_lin = (1. - nu)
                            for i in range(len(cur_q)):
                                if cur_accs[i] >= nu:
                                    daleys[cur_q[i]] = max(1., val_strength * nu_lin / (cur_loss[i] * tau))
                        
                        if kern == 'cos':
                            nu_cos = np.arccos(2 * nu - 1.)
                            for i in range(len(cur_q)):
                                if cur_accs[i] >= nu:
                                    daleys[cur_q[i]] = max(1., val_strength * nu_cos / (np.pi * cur_loss[i] * tau))
                        
                        if kern == 'qua':
                            nu_qua = np.sqrt((1. - nu) / tau)
                            for i in range(len(cur_q)):
                                if cur_accs[i] >= nu:
                                    daleys[cur_q[i]] = max(1., val_strength * nu_qua / cur_loss[i])
                            
                        if kern == 'sec':
                            nu_sec = np.log(1. / nu * (1 + np.sqrt(1 - nu * nu)))
                            for i in range(len(cur_q)):
                                if cur_accs[i] >= nu:
                                    daleys[cur_q[i]] = max(1., val_strength * nu_sec / (cur_loss[i] * tau))

                        # decrease delay for delayed items
                        for item in dey_q:
                            daleys[item] -= 1.
            ...
            if callback_model.stop_training:
                break
        callbacks.on_train_end()
        return self.history, float(n_items_reviewed) / float(len(index_array) * nb_epoch)
    ...
    def _test_loop_rbf(self, f, ins, batch_size=32, verbose=0):
        '''Abstract method to loop over some data in batches.
        ...
        # Returns
            outs: Scalar loss (if the model has a single output and no metrics)
            or list of scalars (if the model has multiple outputs
            and/or metrics). The attribute `model.metrics_names` will give you
            the display labels for the scalar outputs.
            cur_loss: estimated loss of individual instances.
            cur_accs: estimated accuracy of individual instances.
        '''
        nb_sample = ins[0].shape[0]
        ins_loss=[]
        ins_accs=[]
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batches):
            ...
            ins_loss += [batch_outs[0]] * len(batch_ids)
            ins_accs += [batch_outs[1]] * len(batch_ids)
    ...
    def get_optimal_tau_rbf(self, kern, x, val_accs, val_loss, nu):
        if kern == 'gau':
    ...
    def fit(self, x, y, batch_size=32, nb_epoch=10, verbose=1, callbacks=[],
            ...
            class_weight=None, sample_weight=None, kern='rote', nu=0.5):
        ...
        # Arguments
            ...
            kern: type of kernel function, it could be any kernel in 
                ['gau', 'lap', 'lin', 'cos', 'qua', 'sec'] for rbf (these represent
                gaussian, laplace, linear, cosine, quadratic, and secant 
                functions respectively). If kern in not in the above list, network 
                will be trained with standard training.    
            nu = recall confidence for rbf kernels. RbF scheduler estimates the maximum delay 
                such that instances can be recalled with this confidence in 
                the future iterations, nu takes a value in (0,1)  
        ...
        kern = kern.lower().strip()        
        if kern in ['gau', 'lap', 'lin', 'cos', 'qua', 'sec']:
            return self._fit_loop_rbf(f, ins, out_labels=out_labels,
                                  batch_size=batch_size, nb_epoch=nb_epoch,
                                  verbose=verbose, callbacks=callbacks,
                                  val_f=val_f, val_ins=val_ins, shuffle=shuffle,
                                  callback_metrics=callback_metrics, kern=kern, nu=nu)
